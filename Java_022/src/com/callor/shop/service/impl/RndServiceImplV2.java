...
public class RndServiceImplV2 {
	...
	/*
	 * 매개 변수가 없는 생성자와 파일이름을 매개변수로 전달받아야 하는 생성자를
	 * 선언했다.
	 * 
	 * 매개변수가 없는 생성자를 호출하여 객체를 생성하면 
	 * 	( = new RndServiceImplV2() 라고하면 )
	 * 파일이름을 다시 매개변수를 전달하면서
	 *  다시 매개변수가 있는 생성자를 호출한다.
	 */
	public RndServiceImplV2() {
		this("src/com/callor/shop/files/nums_rnd2.txt");
	}
	
	/*
	 * 매개변수가 있는 생성자는 객체를 생성할 때 반드시 파일이름을 전달받아야만 한다
	 * 
	...
	// 저장할 파일 이름을 코드에 포함하지 않고 saveFileRnd() method를 호출하는 곳에서 파일 이름을 전달해 주도록 코드를 변경 
