...
public class CartServiceImplV1 implements CartService {
	 ...
	 * 
	 * ImplV1을 상속받은 ImplV2의 새로운 save...()method 에서 cartList에 저장된 데이터를 사용하려고 시도했다. 
	 * 
	 *  이러한 경우에
	 *  private으로 선언된 변수(객체)는 서로 공유가 되지 않는다.
	 *  상속 받은 클래스의 method 에서 변수(객체)를 공유하려면 부모 클래스에서 변수(객체)를 반드시 protected로 선언해주어야 한다.
	...
	public void inputCart() {
		...
		while (true) {
			...
			if (intPrice < 100) {
				System.out.println("금액은 100 이상 입력하세요");
			} else {
				break;
			}
		}
	...
	/*
	 * private 으로 선언된 method는 현재 클래스에서만 호출이 가능한 method
	 */
	...
	public void printAllCart() {
		...
		for (int i = 0; i < nSize; i++) {
			...
			// cartList의 i번째 요소를 getter하여 cartVO 객체에 임시(for() 명령 내에 있기 때문에) 선언된 CartVO클래스의
			// cartVO 객체에 저장하라
