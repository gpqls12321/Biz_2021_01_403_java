 ...
 * 매입단가[8],  ioIPrice : 합계 금액 계산해야할 데이터 int
 * 판매단가[9],  ioOPrice : int
 * 수량[10]		 ioQty	  : int
 */
public class IolistVO {
	...
	private int ioOPrice; // 판매 단가
	private int ioQty;
	private int ioITotal; // 매입 금액 : 매입단가 * 수량
	private int ioOTotal; // 판매 금액 : 판매단가 * 수량
	
	/*
	 * 기본 생성자
	 * 기본생성자만 필요할 경우는 별도로 만들지 않아도 된다.
	 * 그런데 필드생성자를 한개라도 만들면 기본 생성자도 반드시 만들어 줘야 한다.
	 * 필드생성자를 만들면 기본 생성자가 자동으로 구현되지 않으므로 반드시 만들어 줘야 된다.
	 * 
	 * 기본생성자를 만들지 않고 필드 생성자를 만들면 다음 코드에서 오류가 발생
